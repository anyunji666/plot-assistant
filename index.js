...
(字段标签需保持英文，内容为中文)
Time: \${本轮场景结束时刻，精确到年月日+时分；日期不明可自拟符合背景的纪年}
Location: \${本轮场景最后所在地点}
...
**Relationships**（只写本轮变化，多组分号分隔，按顺序执行）
\`\`\`
...
**Inventory**（只写本轮变化，无变化留空，按顺序执行）
\`\`\`
for 道具变化 in 本轮:
    if not 可随身携带实体物品: 跳过  # 状态/情形归Overview
    if 用尽/丢失/送出: 值 = [REMOVE]
...
Step2 新增：本轮出现值得长线追踪的伏笔/线索/未解约定才记录；关系变化归Relationships，意图、事件叙述和本轮已完成的事件归Overview
