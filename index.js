...
每次回复必须以摘要块结尾，摘要块永远是回复的**最后一个模块**（选项/备注等其他内容放在它前面）。缺失摘要块=输出不合格。
...
for 角色 in 本轮关系有变化的角色:
    if 本轮死亡 or 明确永久离场:
        ...
        值 = 该身份词  # 若同时有阶段性变化，用括号在身份词后补充，如"师徒(暧昧)"
        # 身份/名分词：师徒 / 师兄弟 / 师兄妹 / 师姐妹 / 师姐弟 / 义兄弟 / 义兄妹 / 义姐妹 / 义姐弟 / 同门 / 同学 / 同事 / 邻居 / 网友 / 战友 / 上下级 / 继父子 / 继父女 / 继母子 / 继母女 / 主仆 / 学长学弟 / 学长学妹 / 学姐学弟 / 学姐学妹 / 师生 / 校友 / 室友 / 甲乙方 / 合伙人 / 医患 / 队友 / 教练学员 / 房东租客 / 粉丝偶像
...
# 【值必须是纯净词】只能是：
#   (a) 身份/血亲词本身，如需标注阶段可在词后用括号补充阶段词——如"师徒(暧昧)"
...
# 删除格式同Inventory：角色名·关键词: [REMOVE]
\`\`\`

**Overview**（本轮无实质进展则留空，不超过150字）
\`\`\`
按时间顺序列出本轮关键事件，每件事紧跟其造成的实际改变(关系/处境/认知层面的变化，而非情绪本身)。
平铺直叙，不用比喻/形容词。
\`\`\`
...
function buildFloorRestoreInstruction() {
  return `现在请你逐层还原一段故事历史缺失的摘要字段，不要续写故事，不要输出 <summary> 标签之外的任何文字。

以下对话原文里，每层楼开头都标注了楼层号和说话者（如"[第12楼] AI："或"[第12楼] 用户："）。原文中会穿插用户发言的楼层，这些楼层只是帮你理解上下文的参考，不需要为它们输出任何内容。用户消息里下方会明确列出这次需要还原的目标AI楼层号，请只针对这些楼层分别输出一组 Time/Location/Overview。

还原原则：
- 只处理消息中列出的目标楼层，每一层都要单独输出一组，不能合并多层、不能跳过其中任何一层、也不能把一层拆成多组
- 不要为用户发言的楼层输出任何区块
- Time: 该层故事内的当前日期/时间；需具体到年月日，若文本未提供日期则自拟符合故事背景的纪年/日期
- Location: 该层结束时角色所在的场景地点；若该层内多次转场，写最后停留的地点即可
- Overview: 该层关键事件按时间顺序列出，每件事紧跟其造成的实际改变（关系/处境/认知层面的变化，而非情绪本身）；平铺直叙，不用比喻和形容词；写成一段话不换行；该层若无实质推进则留空；不超过150字
