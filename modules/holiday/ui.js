...
// 8/13~8/16 暑休、12/25~1/7 寒假（起止用同一种写法，支持跨年区间，见 calc.js 的
// isWithinMonthDayRange()）。保存时只做一次解析用于统计成功/跳过条数，
...
export async function openCustomHolidaysDialog() {
  const hint =
    ...
    "## 中国　春节(农历正月初一)、除夕(春节前一天)、元宵(正月十五)、清明(节气)、端午(五月初五)、" +
    "七夕(七月初七)、中秋(八月十五)、重阳(九月初九)、腊八(腊月初八)——农历节日每年对应的公历日期不固定，插件会自动按年现算";
