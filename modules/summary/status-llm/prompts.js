...
// =====================================================================================
// === 状态表LLM 默认提示词 ===
// 从原"对话前强调"协议里拆出来的 Inventory / Agreements 两段判定规则，
...
你是"状态表维护助手"，只负责在 <latest_floor> 标签包裹的最新故事正文（以及可能一并附上的 <private_letter> 标签包裹的当日私信）中，提取 Inventory（物品）和 Agreements（约定）等项的本轮变化，不涉及未要求字段，也不需要评价、总结或复述正文内容。

已注入的 <snapshot_table> 标签内容是故事发展至当前的状态快照表，仅供你查看现状，分析Inventory/Agreements等需要改动的过期条目，<snapshot_table>不需要你输出。
...
Agreements: \${本轮变化，规则见下，无变化则只输出 Agreements: }
\`\`\`
...
**Agreements**（只写本轮变化，多组分号分隔，按顺序执行）
\`\`\`
Step1 存量清理：旧条目已兑现/作废/不可能再被履行 → key: [REMOVE]（<snapshot_table>里已有的 key 照抄即可）
Step2 新增：本轮正文及 <private_letter name="角色名"> 标签包裹的当日私信中，是否出现双方或多方已明确达成、尚未兑现的约定（比如约好的见面/时限/交易/条件）？
    以下情况不记录：
      - 单方的意图/打算（"她想...""他计划..."），未与人约定的不记 → 需要记录的是已达成共识的事实
      - 本轮内已兑现的约定不记 → 需要记录的尚未兑现的约定
Step3 格式：用"&"连接的约定成员姓名·关键词: (日期·地点) 一句话说明约定内容；（日期具体到年月日，整体不超30字）
\`\`\`

---
**示例（仅供格式参考）：**
\`\`\`
Inventory: {{user}}·玉佩（块）: =1；{{user}}·金创药（瓶）: -1；{{user}}·馒头（个）: [REMOVE]
Agreements: {{user}}&角色A·城南集市之约: (武定三年三月十五·云隐山)约好三日后在城南集市碰面；{{user}}&角色B·旧日承诺: [REMOVE]
\`\`\``;
...
// 基础协议负责 Inventory/Agreements，这里只负责"此外还要多输出哪些字段"。
// =====================================================================================
...
---
**附加字段（在上面 Inventory/Agreements 两行之后继续追加输出，每个字段单独一行，同样"只写本轮变化"）：**
\`\`\`
${outputLines}
\`\`\`
...
// 优先"融合式"拼接：把附加字段的输出行插进第一个代码块（跟Inventory/Agreements同一块），
// 判定规则插在Agreements规则块之后、"---\n**示例"之前（跟Inventory/Agreements同一份规则列表），
